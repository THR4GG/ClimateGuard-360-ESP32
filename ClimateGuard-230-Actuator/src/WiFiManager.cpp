...
WiFiManager::WiFiManager(DisplayManager &displayManager)
    : displayManager(displayManager), timeClient(nullptr)
{
}
...
NTPClient &WiFiManager::getTimeClient()
{
    return *timeClient;
}
...
void WiFiManager::checkConnectionAndReconnect()
{
    if (!isWifiConnected())
    {
...
String WiFiManager::convertDecimalToIP(uint32_t decimalIP)
{
    uint8_t octet4 = (decimalIP >> 24) & 0xFF;
    uint8_t octet3 = (decimalIP >> 16) & 0xFF;
    uint8_t octet2 = (decimalIP >> 8) & 0xFF;
    uint8_t octet1 = decimalIP & 0xFF;

    String ipString = String(octet1) + "." + String(octet2) + "." + String(octet3) + "." + String(octet4);
    return ipString;
}
